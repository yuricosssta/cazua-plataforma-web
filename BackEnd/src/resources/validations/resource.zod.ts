...
const sanitizePositiveNumber = z
  .number()
  .positive('A quantidade deve ser maior que zero')
  .transform((val) => Precision.round(val, 2));

// 1. Catálogo
export const createResourceSchema = z.object({
  name: z.string().min(2, 'O nome do recurso é obrigatório'),
  type: z.nativeEnum(ResourceType, {
    required_error: 'Tipo de recurso inválido',
  }),
...
export const addStockSchema = z.object({
  ...
  unitCostSnapshot: z
    ...
    .min(0, 'O custo não pode ser negativo')
    .optional()
    .transform((val) => (val !== undefined ? Precision.round(val, 2) : val)),
  origin: z.string().optional(),
  attachments: z.array(z.string().url('Formato de anexo inválido')).optional(),
});
...
export const cancelTransactionSchema = z.object({
  reason: z
    .string()
    .min(5, 'O motivo do cancelamento é obrigatório e deve ser descritivo'),
});
...
export const approveRequestSchema = z.object({
  approvedQuantity: z
    .number()
    .positive('A quantidade aprovada deve ser maior que zero')
    .transform((val) => Precision.round(val, 2)),
});
