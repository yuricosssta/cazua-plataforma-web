...
import {
  ResourceTransaction,
  ResourceTransactionDocument,
} from '../schemas/resource-transaction.schema';
import { TransactionStatus, TransactionType } from '../types/resource-enums';

@Injectable()
export class ResourceTransactionRepository {
  constructor(
    @InjectModel(ResourceTransaction.name)
    private readonly model: Model<ResourceTransactionDocument>,
  ) {}

  async create(
    data: Partial<ResourceTransaction>,
  ...
  async sanitizeDecimalPrecision(orgId: string): Promise<any> {
    return this.model
      .updateMany({ organizationId: new (Types.ObjectId as any)(orgId) }, [
        {
          $set: {
            quantity: { $round: ['$quantity', 2] },
