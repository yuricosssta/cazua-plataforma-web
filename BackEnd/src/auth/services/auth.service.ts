...
export class AuthService {
  ...
  async signIn(email: string, pass: string): Promise<{ access_token: string }> {
    ...
    if (user.lockUntil && user.lockUntil.getTime() > Date.now()) {
      throw new UnauthorizedException('Muitas tentativas falhas. Conta bloqueada temporariamente.');
    }
    ...
    if (!passwordMatch) {
      const updatedUser = await this.authUsersService.incrementLoginAttempts(email);
      ...
      if (updatedUser && updatedUser.loginAttempts >= this.MAX_LOGIN_ATTEMPTS) {
        ...
        throw new UnauthorizedException(`Muitas tentativas falhas. Conta bloqueada por ${this.LOCK_TIME_MS / 60000} minutos.`);
      }
