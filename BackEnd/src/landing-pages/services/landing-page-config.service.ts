...
export class LandingPageConfigService {
  ...
  async upsertConfig(orgIdString: string, data: UpsertLandingPageDTO) {
    ...
    // Impede que a organização atual assuma um domínio ativo já registrado por terceiros
    const conflict = await this.repository.findConflictingDomain(data.domain, organizationId);
    if (conflict) {
      throw new ConflictException('Este domínio já está em uso por outra organização ativa.');
    }

    // Type assertion bypassa a incompatibilidade shallow do Partial<T> com objetos aninhados do Zod
    return this.repository.upsert(
      organizationId, 
      data as unknown as Partial<LandingPageConfig>
    );
  }
