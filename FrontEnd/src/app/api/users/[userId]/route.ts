...
export async function PUT(request: Request, context: { params: Promise<{ userId: string }> }) {
  ...
  try {
    ...
    const nestResponse = await fetch(`${NEST_API_URL}/users/${userId}`, {
      method: 'PUT',
      headers: {
    ...
    if (!nestResponse.ok) {
      return NextResponse.json({ error: data.message || 'Falha ao atualizar usuário' }, { status: nestResponse.status });
    }
