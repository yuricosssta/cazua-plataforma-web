...
export async function POST(request: Request, context: { params: Promise<{ orgId: string; projectId: string }> }) {
  ...
  try {
    ...
    const nestResponse = await fetch(`${NEST_API_URL}/organizations/${orgId}/resources/return/${projectId}`, {
      method: 'POST',
      headers,
    ...
    if (!nestResponse.ok) {
      return NextResponse.json({ error: data.message || 'Falha ao devolver recurso' }, { status: nestResponse.status });
    }
