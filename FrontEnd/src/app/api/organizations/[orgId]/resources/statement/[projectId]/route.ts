...
export async function GET(request: Request, context: { params: Promise<{ orgId: string; projectId: string }> }) {
  ...
  try {
    const nestResponse = await fetch(`${NEST_API_URL}/organizations/${orgId}/resources/statement/${projectId}`, {
      headers: {
    ...
    return NextResponse.json({ error: 'Erro de comunicaç��o com o back-end' }, { status: 500 });
  }
}
