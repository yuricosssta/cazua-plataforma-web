...
export async function POST(request: Request, context: { params: Promise<{ orgId: string; transactionId: string }> }) {
  ...
  try {
    ...
    const nestResponse = await fetch(`${NEST_API_URL}/organizations/${orgId}/resources/transactions/${transactionId}/approve`, {
      method: 'POST',
      headers,
    ...
    if (!nestResponse.ok) {
      return NextResponse.json({ error: data.message || 'Falha ao aprovar transação' }, { status: nestResponse.status });
    }
