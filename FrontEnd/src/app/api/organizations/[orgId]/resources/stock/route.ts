...
export async function POST(request: Request, context: { params: Promise<{ orgId: string }> }) {
  ...
  try {
    ...
    const nestResponse = await fetch(`${NEST_API_URL}/organizations/${orgId}/resources/stock`, {
      method: 'POST',
      headers,
    ...
    if (!nestResponse.ok) {
      return NextResponse.json({ error: data.message || 'Falha ao adicionar estoque' }, { status: nestResponse.status });
    }
