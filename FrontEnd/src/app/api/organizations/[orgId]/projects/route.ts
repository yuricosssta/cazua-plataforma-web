...
export async function GET(request: Request, context: { params: Promise<{ orgId: string }> }) {
  ...
  try {
    const nestResponse = await fetch(`${NEST_API_URL}/organizations/${orgId}/projects`, {
      headers: {
...
export async function POST(request: Request, context: { params: Promise<{ orgId: string }> }) {
  ...
  try {
    ...
    if (!nestResponse.ok) {
      return NextResponse.json({ error: data.message || 'Falha ao criar projeto' }, { status: nestResponse.status });
    }
